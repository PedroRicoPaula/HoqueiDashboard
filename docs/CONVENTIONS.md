...
**`tax_id_collection` exige `customer_update: { name: 'auto', address: 'auto' }` quando a sessão reutiliza um `customer` existente** (BUG-048, 2026-07-19, só apanhado em produção — em 2 fases, o segundo erro só apareceu depois de corrigir o primeiro) — sem `name` a Stripe rejeita com 400 (`"Tax ID collection requires updating business name on the customer"`); corrigido isso, rejeita outra vez com 400 diferente (`"We could not find a valid address on the provided customer"`) sempre que o `customer` não tem morada gravada (comum aqui — os customers são criados só com `name`+`email`, nunca se pede morada no formulário). Ambos os erros a API devolvia como 500 genérico. Os 5 pontos de checkout do projecto passam sempre um `customer` (novo ou existente), nunca deixam a Stripe criar um anónimo — por isso os 5 precisam sempre dos dois campos a par do `tax_id_collection`.
