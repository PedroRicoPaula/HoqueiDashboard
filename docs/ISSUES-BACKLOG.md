...
### ~~[BUG-048] Checkout falhava com 500 em produção após activar `tax_id_collection`~~ ✅ RESOLVIDO 2026-07-19
**Encontrado:** 2026-07-19, ao vivo em produção, em duas fases — `POST /api/billing/subscribe` (e os outros 4 pontos de checkout) devolvia 500.
- **Fase 1**: `"Tax ID collection requires updating business name on the customer"` — a Stripe exige `customer_update: { name: 'auto' }` sempre que `tax_id_collection` é activado numa Checkout Session que reutiliza um `customer` já existente. Corrigido nos 5 pontos de checkout.
- **Fase 2**, só apareceu depois de corrigir a 1ª (testado ao vivo, num clube de trial real): `"We could not find a valid address on the provided customer"` — falta também `address: 'auto'` no mesmo `customer_update`; os customers deste projecto são criados só com `name`+`email`, nunca com morada. Corrigido nos mesmos 5 pontos: `register`, `billing/subscribe`, `billing/reactivate`, `billing/checkout-link/[clubId]`, `platform/clubs/[id]/send-payment-link`.
**Nota:** não relacionado com saldo Stripe negativo (o utilizador tinha suspeitado disto, por causa de um refund de teste) — confirmado pela mensagem de erro exacta, que é sobre dados em falta no `Customer`, não sobre saldo da conta. Só apanhado em produção porque as chaves Stripe placeholder do `.env.local` falham antes de chegar a este ponto específico da API — não reproduzível em dev local sem chaves reais.
