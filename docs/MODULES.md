...
- **Nav**: logo + nome (`hidden sm:block`), language switcher de texto (`hidden md:flex`) + variante compacta com ícone `Globe` em dropdown (`md:hidden`, `LanguageSwitcherCompact` em `LanguageSwitcher.tsx` — 2026-07-23, antes o idioma só era mudável no rodapé em mobile), login (`hidden sm:inline`), botão registar sempre visível.
...
- **Secção "O produto real"** (`ProductScreenshots.tsx`): fundo escuro, tabs Mensalidades/Atletas, imagens reais do dashboard (`/screenshots/fees-preview.png`, `/screenshots/athletes-preview.png`), frame de browser fake. Usa `<img>` tag (não `next/image`) porque os ficheiros são estáticos em `public/`. **Recapturados 2026-07-23** (clube seed `HC Porto Demo` em dev local) — os anteriores eram de 20 de junho, antes de Season Membership e do redesign de Definições; fecha [UX-005b] em `docs/ISSUES-BACKLOG.md`.
...
- **`hero.subtitle` reescrito (2026-07-23)** — passa a nomear a dor concreta ("chega de folhas de Excel perdidas e de perguntar quem já pagou") em vez de só listar módulos, nos 5 idiomas. Este texto alimenta também a meta `description` (ver abaixo), por isso qualquer edição futura tem de continuar razoável como resumo de pesquisa, não só como copy de hero.
- **Metadata de partilha (2026-07-23)** — `generateMetadata()` em `[locale]/layout.tsx` passou de só `alternates.languages` para `title`/`description` (traduzidos, derivados de `hero.title`+`hero.subtitle` via `getTranslations`) + `openGraph` (imagem `/og-image.png`, 1200×630, `locale`/`alternateLocale` por idioma) + `twitter` (`summary_large_image`). Antes, partilhar o link (WhatsApp, Slack, etc.) mostrava só o título genérico do `layout.tsx` raiz, sem imagem. `public/og-image.png` é uma imagem estática única (não uma por idioma) — só o texto à volta é traduzido.
