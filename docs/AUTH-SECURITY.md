...
- `POST /api/billing/reactivate` → 10 req / 60 min por IP
- `GET /api/billing/checkout-link/[clubId]` → 30 req / 60 min por IP (2026-07-19)
...
- **`POST /api/billing/reactivate`** (público — chamado a partir do próprio ecrã de login bloqueado, onde por definição ainda não há sessão) — rate limited 10/hora/IP. Só aceita clubes **não-grátis** em `SUSPENDED`/`PAST_DUE`; devolve erro claro ("contacte o suporte") para clubes grátis, sem crashar. Reabre Stripe Checkout — reutiliza `stripeCustomerId` se existir, senão cria um na hora (2026-07-19: cobre trials expirados, que nunca chegaram a ter customer Stripe). `success_url`/`cancel_url` apontam sempre para `/login` (nunca faz login automático, ao contrário do fluxo de registo).
...
### Free trial de 14 dias (2026-07-19)

- **`POST /api/register`** com `plan: "trial"` — ramo totalmente separado do fluxo pago, nunca toca na Stripe. Cria `Club` já `ACTIVE` com `trialEndsAt = now + 14 dias`, login automático (mesmo contrato JSON dos outros fluxos de registo). Rate limit partilhado com o resto do endpoint (5/hora/IP).
- **`POST /api/billing/subscribe`** (autenticado, `isAdmin`, só o próprio clube — sem parâmetro de ID na rota, usa o `clubId` do JWT) — self-serve, chamado a partir do card "Plano" em `/settings`. Cria/reutiliza `stripeCustomerId`, gera Stripe Checkout, `success_url` volta directo para `/settings?upgraded=1` (já autenticado, ao contrário de `reactivate`/`checkout-link` que voltam para `/login`).
- **`GET /api/billing/checkout-link/[clubId]?plan=`** (público, rate limited 30/hora/IP) — usado nos 2 links do email de boas-vindas do trial. Gera a sessão Stripe **no momento do clique** e redirige (302) — nunca embutir a URL de uma sessão já criada num email de longa duração: Checkout Sessions expiram ao fim de 24h no máximo, limite fixo da própria API Stripe, não contornável via `expires_at`. Sem autenticação — protecção é validar que o clube existe, não é grátis, e ainda não tem subscrição activa (senão redirige para `/login?already=1`).
- **`GET /api/cron/trial-sweep`** — protegido por `CRON_SECRET`, **não** pelo middleware/CSRF habitual (é chamado pela própria Vercel, não por um browser). A Vercel injecta automaticamente `Authorization: Bearer <CRON_SECRET>` quando essa env var está definida no projecto — a rota compara o header recebido com `process.env.CRON_SECRET`, 401 se não bater certo ou a env var não estiver definida. Configurado em `vercel.json` (`crons`), 1×/dia — no plano Hobby da Vercel é o máximo permitido, e a Vercel pode invocar a qualquer momento dentro da hora indicada (não ao minuto exacto). Idempotente por natureza (`updateMany` só afecta quem ainda está `ACTIVE`) — desenhado para tolerar invocações em falta ou duplicadas, que a própria Vercel documenta como best-effort, não garantido.
