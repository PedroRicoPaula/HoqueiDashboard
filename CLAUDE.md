...
15. **Cancelamento/suspensão/reactivação de subscrição (2026-07-18)** — `CANCELLED` e `SUSPENDED` convergem num único estado operacional: **`SUSPENDED`** é o estado final tanto para cancelamento self-serve como para falha de pagamento definitiva (`customer.subscription.deleted`). `CANCELLED` continua no enum `ClubStatus` por compatibilidade, mas deixou de ser atribuído por qualquer fluxo novo — não reintroduzir sem rever `POST /api/billing/cancel`, `POST /api/billing/reactivate` e o webhook. Três rotas nunca chamam `getStripe()` sem checar antes se as chaves são placeholder (não crasham, falham com erro controlado do Stripe):
    ...
    - **Eliminação permanente (regra simplificada 2026-07-19)**: qualquer clube `SUSPENDED` — grátis ou pago, sem espera — botão em `/platform` (`src/app/api/platform/clubs/[id]/route.ts`, `DELETE`, confirma email exacto do clube antes de activar). O limite de "1 ano em `SUSPENDED`" que existia só para clubes pagos foi removido (`statusChangedAt` continua gravado, mas já não bloqueia a eliminação) — `SUSPENDED` já significa "não está a ser pago" independentemente de como o clube nasceu. Não há eliminação automática — sempre acção manual do super admin.
...
### Landing atualizada + free trial visível + login/registo multilanguage (2026-07-19)
> Pedido do utilizador: landing "toda bem atualizada", com o free trial visível na própria mensagem (implementado na ronda anterior mas invisível na landing), e garantir que login/registo também são multilingue — não só a landing.
- ✅ **Features da landing: 6→12 módulos** — `featureKeys` em `src/app/[locale]/page.tsx` só mostrava 6 dos 12 módulos vendidos (subtítulo já dizia "12 módulos integrados", undersell confirmado — [UX-005], agora resolvido em `docs/ISSUES-BACKLOG.md`). Adicionados Mensalidades, Patrocinadores, Direção, Assiduidade, Relatórios, Permissões nos 5 idiomas
- ✅ **Trial na mensagem da landing** — `hero.ctaSub` passa a mencionar "14 dias grátis, sem cartão de crédito"; novo link `pricing.trialCta` sob o card de preço (`/${locale}/register?plan=trial`, pré-seleciona o plano); nova pergunta na FAQ, primeiro item da lista. 5 idiomas
- ✅ **`?plan=` no registo** — `src/app/[locale]/register/page.tsx` lê `plan=trial|monthly|yearly` de `window.location.search` no mount (mesmo padrão de `?upgraded=1` em Definições, sem `useSearchParams` para não obrigar a Suspense boundary)
- ✅ **`/login`, `/forgot-password`, `/reset-password`, `/register/complete` multilingue** — únicas páginas públicas ainda 100% em português fixo (não vivem sob `[locale]/`, mudar isso implicaria reescrever middleware + todos os redirects `/login` hardcoded, desproporcional). Novo hook `useAuthT()` (`src/hooks/useAuthT.ts`) + `messages/auth/{pt,en,es,fr,it}.json` + `<AuthLanguageSwitcher>`. Detalhe completo em `docs/CONVENTIONS.md` → "Auth pages i18n"
- ✅ **Handoff de idioma sem URL prefixada** — `?lang=` nos links `/login` da landing e nos `success_url` de `billing/reactivate`/`billing/checkout-link`/`send-payment-link` (usam `club.language`) e do registo pago para `/register/complete` (usa `language` do formulário); fallback `localStorage['hm-locale']` → idioma do browser → PT
- ✅ **Bug apanhado ainda durante a implementação, não só depois**: `useAuthT()` com `useState` local fazia `LoginPage` e `LoginForm` (esta última dentro de `<Suspense>`) terem cada uma o seu próprio idioma — o switcher só mudava o texto de uma das duas. Corrigido com `useSyncExternalStore` (estado module-level partilhado por todas as chamadas ao hook). Confirmado ao vivo via Playwright: troca ES→EN actualiza título, labels e placeholders em simultâneo nas duas partes da página
