...
export async function PUT(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const member = await prisma.directionMember.update({ where: { id }, data })

    await logAudit(req, user.id, user.email, 'UPDATE', 'DirectionMember', member.id, { name: member.name })
    return NextResponse.json(member)
  } catch (error: unknown) {
...
export async function DELETE(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    await prisma.directionMember.delete({ where: { id } })
